...
const promotions = [
  { title: '50% Discount on Accessories', offer: '50% OFF', image: 'https://images.unsplash.com/photo-1505740420928-5e560c06d30e?auto=format&fit=crop&w=900&q=85', alt: 'Premium over-ear headphones', kind: 'accessories' },
  { title: '5% Discount on Mobiles', offer: '5% OFF', image: 'https://images.unsplash.com/photo-1598327105666-5b89351aff97?auto=format&fit=crop&w=900&q=85', alt: 'Modern premium smartphone', kind: 'mobiles' },
  { title: 'Neck Band', offer: 'Buy @ ₹149/-', image: 'https://images.unsplash.com/photo-1631176093617-63490a3d785a?auto=format&fit=crop&w=900&q=85', alt: 'Wireless neckband earphones', kind: 'neckband' },
  { title: 'TWS Buds', offer: 'Buy @ ₹399/-', image: 'https://images.unsplash.com/photo-1606220945770-b5b6c2c55bf1?auto=format&fit=crop&w=900&q=85', alt: 'Modern true wireless earbuds', kind: 'buds' },
  { title: 'Smart Watch', offer: 'Buy @ ₹799/-', image: 'https://images.unsplash.com/photo-1546868871-7041f2a55e12?auto=format&fit=crop&w=900&q=85', alt: 'Premium smartwatch', kind: 'watch' },
  { title: 'Glass Protection', offer: 'Buy @ ₹49/-', image: 'https://images.unsplash.com/photo-1556656793-08538906a9f8?auto=format&fit=crop&w=900&q=85', alt: 'Mobile screen with glass protection', kind: 'glass' },
] as const

const rewardPresentation: Record<string, { label: string; offer: string; image: string; alt: string; description: string }> = {
  'accessories-50': { label: '50% OFF', offer: '50% OFF', image: promotions[0].image, alt: promotions[0].alt, description: 'Save 50% on eligible Mobile Hub accessories.' },
  'mobiles-5': { label: '5% MOBILES', offer: '5% OFF', image: promotions[1].image, alt: promotions[1].alt, description: 'Save 5% on your next premium mobile purchase.' },
  'neckband-149': { label: '₹149 NECKBAND', offer: '₹149/-', image: promotions[2].image, alt: promotions[2].alt, description: 'Take home a wireless neckband for the special price of ₹149/-.' },
  'tws-399': { label: '₹399 TWS', offer: '₹399/-', image: promotions[3].image, alt: promotions[3].alt, description: 'Take home modern TWS Buds for the special price of ₹399/-.' },
  'watch-799': { label: '₹799 WATCH', offer: '₹799/-', image: promotions[4].image, alt: promotions[4].alt, description: 'Get a premium Smart Watch for the special price of ₹799/-.' },
  'glass-49': { label: '₹49 GLASS', offer: '₹49/-', image: promotions[5].image, alt: promotions[5].alt, description: 'Protect your mobile screen with glass protection for ₹49/-.' },
}
...
const pointOnWheelAt = (angle: number, radius: number) => {
  const radians = (angle * Math.PI) / 180
...
function App() {
  ...
  const rewardView = reward ? rewardPresentation[reward.id] : null
  ...
  return <main>
    ...
    <section className="app-card" aria-live="polite">
      ...
      {step === 'spin' && <><div className="step-label">STEP 4 OF 4</div><h2>It’s your lucky moment!</h2><p className="muted">Tap the wheel or button and let the backend decide.</p><div className="wheel-area premium-wheel-area"><div className="premium-pointer" aria-hidden="true"><i /></div><div className="wheel-assembly"><div className="wheel premium-wheel" style={{ transform: `rotate(${rotation}deg)` }}><svg viewBox={`0 0 ${WHEEL_SIZE} ${WHEEL_SIZE}`} role="img" aria-label="Six reward promotional spin wheel"><defs><radialGradient id="chrome" cx="30%" cy="25%"><stop offset="0" stopColor="#fff" /><stop offset=".22" stopColor="#94999c" /><stop offset=".48" stopColor="#f8f8f5" /><stop offset=".72" stopColor="#62676d" /><stop offset="1" stopColor="#e4e6e5" /></radialGradient><filter id="wheelShadow"><feDropShadow dx="0" dy="9" stdDeviation="7" floodOpacity=".34" /></filter>{prizes.map((_, index) => <clipPath id={`segment-clip-${index}`} key={index}><path d={segmentPath(index)} /></clipPath>)}</defs><circle cx="150" cy="150" r="148" fill="url(#chrome)" filter="url(#wheelShadow)" /><circle cx="150" cy="150" r="138" fill="#751019" />{prizes.map((prize, index) => { const middleAngle = index * SEGMENT_ANGLE + SEGMENT_ANGLE / 2; const [imageX, imageY] = pointOnWheelAt(middleAngle - 90, 87); const [textX, textY] = pointOnWheelAt(middleAngle - 90, 119); const presentation = rewardPresentation[prize.id]; return <g key={prize.id}><path d={segmentPath(index)} fill={index % 2 === 0 ? '#c70f21' : '#fff8ee'} stroke="#751019" strokeWidth="2" /><g clipPath={`url(#segment-clip-${index})`}><image href={presentation.image} x={imageX - 21} y={imageY - 21} width="42" height="42" preserveAspectRatio="xMidYMid slice" className="wheel-product-image" /></g><g transform={`translate(${textX} ${textY}) rotate(${middleAngle})`}><text className={index % 2 === 0 ? 'wheel-svg-text light' : 'wheel-svg-text dark'} textAnchor="middle" y="3">{presentation.label}</text></g></g>})}<circle cx="150" cy="150" r="55" fill="url(#chrome)" stroke="#641019" strokeWidth="5" /></svg></div><button className="center-spin" type="button" onClick={spin} disabled={spinning} aria-label="Spin the wheel">{spinning ? '...' : 'SPIN'}</button></div></div><button className="primary spin-button" type="button" onClick={spin} disabled={spinning}>{spinning ? 'Spinning...' : 'SPIN NOW'} <span>{spinning ? '✦' : '⟳'}</span></button><p className="fine-print">One spin per mobile number. Your result is securely saved by Mobile Hub.</p></>}
      {step === 'result' && reward && rewardView && <div className="celebration-popup" role="dialog" aria-label="Spin result"><div className="result-top">{alreadyPlayed ? 'YOUR EARLIER REWARD' : 'YOUR REWARD'}</div><div className="confetti" aria-hidden="true">✦ &nbsp; ✦ &nbsp; ✦</div><h2>{alreadyPlayed ? 'Welcome back!' : 'Congratulations!'}</h2><p className="muted">{alreadyPlayed ? 'Your reward is safely saved below.' : 'You unlocked a Mobile Hub offer.'}</p><article className="reward-card"><div className="reward-image-wrap"><img src={rewardView.image} alt={rewardView.alt} /></div><div className="reward-copy"><span>{alreadyPlayed ? 'YOUR REWARD' : 'YOU WON'}</span><strong>{rewardView.offer}</strong><h3>{reward.title}</h3><p>{rewardView.description}</p></div></article><p className="claim">Show this screen at Mobile Hub to claim your offer.</p><button className="secondary reward-done" type="button" onClick={restart}>Done <span>✓</span></button></div>}
    </section>
    <section className="promotions" aria-labelledby="promotions-title">
      <div className="promotions-heading"><p className="eyebrow">MOBILE HUB SPECIALS</p><h2 id="promotions-title">Featured offers</h2><p>Great everyday deals, picked for your next upgrade.</p></div>
      <div className="promotion-grid">
        {promotions.map((promotion) => <article className={`promotion-card ${promotion.kind}`} key={promotion.title}>
          <div className="promotion-image"><img src={promotion.image} alt={promotion.alt} loading="lazy" /></div>
          <div className="promotion-details"><span className="promotion-offer">{promotion.offer}</span><h3>{promotion.title}</h3></div>
        </article>)}
      </div>
    </section>
